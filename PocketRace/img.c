...
void showImage(u16 offset,SOD_IMG* img,u8 x,u8 y)
{
  ...
  for (j=0;j<19;j++)
  {
      for (i=0;i<20;i++)
      {
          (SCROLL_PLANE_1)[j*32+i] = offset;
          (SCROLL_PLANE_2)[j*32+i] = offset;
      }
  }
  ...
  for (i=0;i<img->h;i++)
  {
      for (j=0;j<img->w;j++)
      {
          ...
          (SCROLL_PLANE_1)[(i+y)*32+j+x] = (a<<9)+(offset+1+i*img->w+j);
          a = img->pi2[i*img->w+j]+1;
          (SCROLL_PLANE_2)[(i+y)*32+j+x] = (a<<9)+(offset+1+i*img->w+j+l);
      }
  }
