...
#define LINK_HANDSHAKE 0xF0
#define LINK_GAME 0xF1
#define LINK_META 0xF2
...
void setMainSprite(u8 n,u8 i,u8 duel)
{
    u16 n0,n1,t0,t1;
    u8 x = chars[i].x-scr_x;
    u8 y = chars[i].y-scr_y;
...
void youLose(const char *you, const char* lose)
{
    u8 j;
...
u8 initLink() {
    ...
    ngpc_linkkit_set_role(LINKKIT_ROLE_AUTO);
    ngpc_linkkit_init();
    ...
    while((!ReadyHost)||(!ReadyClient))
    {
        ...
        linkState = ngpc_linkkit_state();
        switch (linkState) // Check link status
        {
            case LINKKIT_CONNECTING:
                print("CONNECTING", 0, 8);
                break;
            case LINKKIT_READY:
...
static void duelInitChars(u8 host0)
{
    u8 i;
    /* Host player top-left, client player bottom-right */
    chars[0].alive = 1;
    chars[0].idx   = 0;
    chars[0].x     = host0 ? 16 : 208;
    chars[0].y     = host0 ? 16 : 208;
    chars[0].speed = 0;
    chars[0].power = 1;
    chars[0].nbmax = 1;
    chars[0].nb    = 0;
    chars[0].dir   = host0 ? J_DOWN : J_UP;
    chars[0].anim  = 0;
    chars[0].count = 4;
    chars[0].tempo = 0;
    chars[0].invers= 0;
    ...
    chars[1].alive = 1;
    chars[1].idx   = 0;
    chars[1].x     = host0 ? 208 : 16; /* tile 13 * 16 */
    chars[1].y     = host0 ? 208 : 16;
    chars[1].speed = 0;
    chars[1].power = 1;
    chars[1].nbmax = 1;
    chars[1].nb    = 0;
    chars[1].dir   = host0 ? J_UP : J_DOWN;
    chars[1].anim  = 0;
    chars[1].count = 4;
    chars[1].tempo = 0;
    chars[1].invers= 0;
...
void duel()
{
    ...
    u8 lastRemoteBombSeq = 0;  /* last remote bomb-seq value we have already placed   */
    ...
    while (1)
    {
        ...
        if (ngpc_linkkit_fresh())
        {
            ...
            if (theirBuffer[0] == LINK_GAME)
            {
                ...
                if (chars[1].alive)
                {
                    ...
                    chars[1].dir = theirBuffer[3] & 0xf;
                    chars[1].count = theirBuffer[4] & 0xf;
                    chars[1].anim = (theirBuffer[4] >> 4) & 0xf;
                }

                /* ---- remote bomb placement (latched, coord-exact) ----
                 * Act only when the peer's counter advanced past what we last
                 * placed. A dropped frame is harmless: the counter is retrans-
                ...
                if (rSeq != 0 && rSeq != lastRemoteBombSeq)
                {
                    ...
                    {
                        /* NO_OWNER: mngBombs() never adjusts any chars[].nb, so
                         * the two consoles cannot drift on bomb ownership */
                        addBomb(NO_OWNER, 16 - (bp << 1), bx, by, bp);
                    }
                }
            }
        }
        ...
        if (linkState != newLinkState)
        {
            linkState = newLinkState;
            if (newLinkState == LINKKIT_LOST)
            {
    			clearSprites(0);
				youLose("LINK", "LOST");
                return;
