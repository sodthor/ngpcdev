...
u16 dacBufferL[DAC_BUFFERSIZE];
int dacRBufferRead, dacRBufferWrite, dacRBufferCount;
u16 dacBufferR[DAC_BUFFERSIZE];
...
static u32 VolTable[16];
static u32 UpdateStep = 0;	//Number of steps during one sample.
...
void dac_mixer(u16* stream, int length_bytes)
{
	...
	{
		SDL_MixAudio((Uint8*)stream, (Uint8*)&dacBufferR[dacLBufferRead], (DAC_BUFFERSIZE - dacRBufferRead) * 2, volume); //mix it to the buffer
		SDL_MixAudio((Uint8*)&stream[DAC_BUFFERSIZE - dacRBufferRead], (Uint8*)dacBufferR, length_bytes - ((DAC_BUFFERSIZE - dacRBufferRead) * 2), volume); //mix it to the buffer
		dacRBufferRead = length_words - (DAC_BUFFERSIZE - dacRBufferRead);
	}
	else
	{
		SDL_MixAudio((Uint8*)stream, (Uint8*)&dacBufferL[dacRBufferRead], length_bytes, volume); //mix it to the buffer
...
void dac_update(u16* dac_buffer, int length_bytes)
{
...
u8 blockSound[CHIPBUFFERLENGTH], blockDAC[CHIPBUFFERLENGTH];		// Gets filled with sound data.
...
void mixaudioCallback(void *userdata, Uint8 *stream, int len)
{
	...
	if (!psg_mode) {
		...
		for (i = 0; i < 15; ++i)
		{
			ngpSoundInterrupt();
			v = MEM_Z80_RAM[0xde];
			while (MEM_Z80_RAM[0xde] == v)
				z80Step();
		}
	}

    sound_update((u16*)blockSound, len);	//Get sound data
//    memcpy(stream, blockSound, len);        //put it in the buffer
    ...
    {
        //dac_update((u16*)blockDAC, len);	//Get DAC data
        //SDL_MixAudio(stream, blockDAC, len, volume); //mix it to the buffer

        dac_mixer((u16*)stream, len);	//Get DAC data
    }
}
